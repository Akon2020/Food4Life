...
export const getCommentairesParBlog = async (req, res, next) => {
  try {
    ...
    const { includeAll } = req.query; // Query param pour inclure tous les commentaires

    let query;

    if (includeAll) {
      // Pour l'admin: retourne TOUS les commentaires (incluant ceux en attente)
      query = Commentaire.findAll({
        where: { idBlog },
        include: [
          {
            model: Commentaire,
      ...
      // Pour le public: retourne seulement les commentaires approuvés
      query = Commentaire.scope(["approuves", "racine"]).findAll({
        where: { idBlog },
        include: [
          {
            model: Commentaire,
  ...
  try {
    ...
    if (!idBlog || !nomComplet || !email || !contenu) {
      return res.status(400).json({
        message:
          "Champs obligatoires manquants : idBlog, nomComplet, email, contenu",
      });
    }
    ...
    const nouveauCommentaire = await Commentaire.create({
      ...
      contenu,
      statut: "attente",
      typeCommentaire: idUtilisateur ? "utilisateur" : "visiteur",
...
export const modererCommentaire = async (req, res, next) => {
  try {
    ...
    return res
      .status(200)
      .json({ message: "Commentaire modéré avec succès", commentaire });
  } catch (error) {
